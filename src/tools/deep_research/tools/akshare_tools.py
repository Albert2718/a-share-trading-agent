...
from ..schemas import StockCandidate
...
class AkshareTools:
    ...
    def candidates_from_codes(self, codes: List[str]) -> List[StockCandidate]:
        try:
            names = self.stock_names()
        except Exception:
            names = {}
        return [
            StockCandidate(code=normalize_a_share_code(code), name=names.get(normalize_a_share_code(code), ""))
            for code in codes
        ]
    ...
    def hot_candidates(self, top: int = 20) -> List[StockCandidate]:
        def loader():
            ak = self._require_akshare()
        ...
        for row in records[:top]:
            ...
            if code:
                candidates.append(StockCandidate(code=normalize_a_share_code(str(code)), name=str(name or "")))
        return candidates
