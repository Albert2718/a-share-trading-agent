...
from ..utils import clamp, normalize_a_share_code, safe_float
...
class SentimentAnalyst:
    ...
    def _safe_hot(self) -> List[StockCandidate]:
        try:
    ...
    def _build_report(
        ...
        hot_candidates: List[StockCandidate],
    ) -> SentimentReport:
        norm = normalize_a_share_code(candidate.code)
        hot_rank = next((idx + 1 for idx, item in enumerate(hot_candidates) if item.code == norm), None)
        attention_score = 0
        heat_sources = []
