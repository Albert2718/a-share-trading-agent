    ...
    def analyze(
        ...
        for candidate in candidates:
            ...
            fundamental_report = (
                ...
                else FundamentalReport(code=candidate.code, name=candidate.name, status="unavailable", error="skipped by CLI")
            )
            news_report = (
                self._safe_analyze(
                    self.news,
                ...
                if include_news
                else NewsReport(code=candidate.code, name=candidate.name, status="unavailable", error="skipped by CLI")
            )
            sentiment_report = (
                self._safe_analyze(
                    self.sentiment,
                ...
                else SentimentReport(code=candidate.code, name=candidate.name, status="unavailable", error="skipped by CLI")
            )
            try:
